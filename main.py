...
def get_doc_owner_name(user_doc_number):
    ...
    if doc_exist:
        for current_document in documents:
            doc_number = current_document['number']
            if doc_number == user_doc_number:
                current_name = current_document['name']
    return current_name
...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            user_doc_number = input('Введите номер документа - ')
            deleted = delete_doc(user_doc_number)
            if deleted[1]:
                print('Документ с номером "{}" был успешно удален'.format(user_doc_number))
            else:
                print(f"Документ с номером {user_doc_number} не существует")
        elif user_command == 'm':
            move_doc_to_shelf()
